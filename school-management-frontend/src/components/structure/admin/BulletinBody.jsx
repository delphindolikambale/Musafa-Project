        ...
        <div className="relative w-full text-black font-serif text-[10px] leading-tight print:text-black">
            ...
            {header?.watermarkLogoPath && (
                <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none">
                    <img 
                        src={`http://localhost:8080/${header.watermarkLogoPath}`} 
            ...
            <table className="relative z-10 w-full border-collapse border-2 border-black text-center table-fixed bg-transparent">
                ...
                {isEB && (
                    <thead>
                        <tr className="border-b-2 border-black text-[10px] font-bold uppercase bg-white/80">
                            ...
                            <th rowSpan={3} className="border-r-[3px] border-r-black p-0.5 text-[9px] align-middle font-black w-12 bg-white/80">
                                TOTAL<br/>GENERAL
                            </th>
                        ...
                        <tr className="border-b border-black text-[8px] font-bold bg-white/80">
                        ...
                        <tr className="border-b-2 border-black text-[8px] font-bold bg-white/80">
                ...
                <tbody className="bg-transparent">
                    ...
                    {isEB && domains?.map((domain, dIdx) => (
                        <React.Fragment key={`eb-dom-${dIdx}`}>
                            <tr className="border-b border-black font-black text-left text-[9px] bg-gray-200/80 print:bg-gray-200/80">
                            ...
                            {domain.subjects?.map((sub, sIdx) => (
                                <tr key={`eb-sub-${sIdx}`} className="border-b border-black text-[10px] bg-transparent">
                                    <td className="border-r-2 border-black p-1 text-left pl-3 font-normal leading-tight bg-white/40">{sub.subjectName}</td>
                                    
                                    {/* S1 */}
                                    <td className="border-r border-black p-0.5 font-bold bg-white/40">{sub.maxPeriod || '10'}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p1 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p2 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 font-bold bg-white/40">{sub.maxExam || '20'}</td>
                                    <td className="border-r-2 border-black p-0.5 font-bold bg-white/40">{sub.totalS1 ?? ''}</td>
                                    
                                    {/* S2 */}
                                    <td className="border-r border-black p-0.5 font-bold bg-white/40">{sub.maxPeriod || '10'}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p3 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p4 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 font-bold bg-white/40">{sub.maxExam || '20'}</td>
                                    <td className="border-r-2 border-black p-0.5 font-bold bg-white/40">{sub.totalS2 ?? ''}</td>
                                    
                                    <td className="border-r-[3px] border-r-black p-0.5 font-black bg-white/40">{sub.totalAnnuel ?? ''}</td>
                                    <td className="border-r border-black p-0.5 font-bold bg-white/40">{sub.repechagePct ?? ''}</td>
                                    <td className="p-0.5 bg-white/40"></td>
                                </tr>
                            ))}
                            {/* Sous-Total du Domaine */}
                            <tr className="border-b-2 border-black font-bold text-[9.5px] bg-gray-100/80">
                    ...
                    {isHumanites && (
                        <>
                            {/* LIGNE DES MAXIMA */}
                            <tr className="border-b-2 border-black font-black text-[10px] bg-gray-200/90 print:bg-gray-200/90">
                            ...
                            {allHumanitesSubjects.map((sub, sIdx) => (
                                <tr key={`hum-sub-${sIdx}`} className="border-b border-black text-[10px] bg-transparent">
                                    ...
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p1 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p2 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.maxExam ? (sub.examS1 ?? '') : ''}</td>
                                    <td className="border-r-2 border-black p-0.5 font-bold bg-white/40">{sub.totalS1 ?? ''}</td>
                                    
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p3 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.p4 ?? ''}</td>
                                    <td className="border-r border-black p-0.5 bg-white/40">{sub.maxExam ? (sub.examS2 ?? '') : ''}</td>
                                    <td className="border-r-2 border-black p-0.5 font-bold bg-white/40">{sub.totalS2 ?? ''}</td>
                    ...
                    {/* 1. MAXIMA GENERAUX */}
                    <tr className="border-b border-black font-black uppercase bg-white/90 text-[9.5px]">
                    ...
                    {/* 2. TOTAUX OBTENUS */}
                    <tr className="border-b border-black font-bold uppercase bg-white/90 text-[10px]">
                    ...
                    {/* 3. POURCENTAGE */}
                    <tr className="border-b border-black font-bold uppercase bg-white/90 text-[9.5px]">
                    ...
                    <tr className="border-b-2 border-black font-bold uppercase bg-white/90 text-[9.5px] h-7">
